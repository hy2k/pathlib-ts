...
const apiExtractorConfig = docsRoot.joinpath("api-extractor.json").toString();
const apiExtractorTemp = docsRoot.joinpath("temp").toString();

const packageName = "pathlib-ts";
const docsPackageName = "pathlib-ts-docs";

if (!(await outDir.exists())) {
	await outDir.mkdir({ parents: true, existOk: true });
}

console.log("Install dependencies for workspace package...");
const install = spawnSync("bun", ["install", "--frozen-lockfile"], {
	stdio: "inherit",
	cwd: repoRoot.toString(),
});
if (install.status !== 0) {
	console.error(`bun install failed for ${packageName}`);
...
if (build.status !== 0) {
	console.error(`bun run build failed for ${packageName}`);
...
for (const file of await outDir.iterdir()) {
	...
	await file.writeText(rewriteMarkdown(markdown));
}
...
function buildSlugSegment(stem: string): string {
	const prefix = `${docsPackageName}.`;
	let value = stem;
	if (value.startsWith(prefix)) {
		const remainder = value.slice(prefix.length);
		value = remainder.length > 0 ? remainder : value;
	} else if (value === docsPackageName) {
		value = packageName;
	}

	return slugify(value) || slugify(stem);
}

function rewriteMarkdown(markdown: string): string {
	return rewriteMarkdownLinks(removeBreadcrumbs(markdown)).replaceAll(
		docsPackageName,
		packageName,
	);
}
